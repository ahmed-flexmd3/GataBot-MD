...
global.authFileJB  = 'NIGHTSESSION'
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "NIGHTSESSION"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
const subBotDirectory = './NIGHTSESSION';
...
(async () => {
global.conns = [];
const mainBotAuthFile = 'NIGHTSESSION';
try {
...
try {
const listaDirectorios = readdirSync('./NIGHTSESSION/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./NIGHTSESSION/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./NIGHTSESSION/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./NIGHTSESSION/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./NIGHTSESSION/', './NIGHTSESSION/']
